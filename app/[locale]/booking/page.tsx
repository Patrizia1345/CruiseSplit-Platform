...
const STEP_KEYS = ["segment", "travellers", "payment"] as const;
    ...
    <div className="grid grid-cols-3 gap-3">
      {Object.entries(cabins).map(([cabin, price]) => (
        <button key={cabin} onClick={() => onSelect(cabin)}
          ...
          <span className="text-xs text-gray-400">{t("cabin.perPerson")}</span>
        </button>
      ))}
    </div>
  );
...
function BookingContent() {
  ...
  function validateTravellers() {
    ...
    travellers.forEach((tr, i) => {
      if (!tr.firstName) newErrors[`firstName_${i}`] = t("step2.errorRequired");
      if (!tr.lastName) newErrors[`lastName_${i}`] = t("step2.errorRequired");
      if (!tr.birthDate) newErrors[`birthDate_${i}`] = t("step2.errorRequired");
    });
    setErrors(newErrors);
  ...
  function validatePayment() {
    const newErrors: Record<string, string> = {};
    if (!payment.email || !payment.email.includes("@")) newErrors.email = t("step3.errorEmail");
    if (!payment.cardName) newErrors.cardName = t("step2.errorRequired");
    if (!payment.cardNumber || payment.cardNumber.replace(/\s/g, "").length < 16) newErrors.cardNumber = t("step3.errorCard");
    if (!payment.expiry) newErrors.expiry = t("step2.errorRequired");
    if (!payment.cvv || payment.cvv.length < 3) newErrors.cvv = t("step2.errorRequired");
    if (!payment.acceptTerms) newErrors.acceptTerms = t("step3.errorTerms");
    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  }
        ...
        <div className="flex gap-6 items-start">
          <div className="flex-1">
            ...
            {step === 0 && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <h2 className="text-xl font-bold mb-1" style={{ color: "#0A2342" }}>{t("step1.heading")}</h2>
                <p className="text-sm text-gray-400 mb-6">{t("step1.subtitle")}</p>
                ...
                <div className="mb-6">
                  <label className="text-sm font-semibold text-gray-700 mb-3 block">{t("step1.cabinCategory")}</label>
                  <CabinSelector cabins={segment.cabins} selected={cabin} onSelect={setCabin} />
                </div>
                <div className="mb-8">
                  <label className="text-sm font-semibold text-gray-700 mb-3 block">{t("step1.numberOfTravellers")}</label>
                  <div className="flex items-center gap-4">
                    ...
                    <button onClick={() => setPersons((p) => Math.min(6, p + 1))}
                      className="w-10 h-10 rounded-full border-2 border-gray-200 text-gray-600 font-bold text-lg hover:border-[#0EA5E9] hover:text-[#0EA5E9] transition-colors">+</button>
                    <span className="text-sm text-gray-400 ml-2">{t(persons === 1 ? "step1.personOne" : "step1.personOther", { count: persons })}</span>
                  </div>
                </div>
            ...
            {step === 1 && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <h2 className="text-xl font-bold mb-1" style={{ color: "#0A2342" }}>{t("step2.heading")}</h2>
                <p className="text-sm text-gray-400 mb-6">{persons === 1 ? t("step2.subtitleOne") : t("step2.subtitleOther", { count: persons })}</p>
                {travellers.map((tr, i) => (
                  <div key={i} className="mb-6 p-4 rounded-xl border border-gray-100 bg-gray-50">
                    <h3 className="text-sm font-semibold text-gray-700 mb-4">
                      {t("step2.traveller", { n: i + 1 })} {i === 0 && <span className="text-xs text-gray-400">{t("step2.mainContact")}</span>}
                    </h3>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs text-gray-500 mb-1 block">{t("step2.firstName")}</label>
                        <input type="text" value={tr.firstName} onChange={(e) => updateTraveller(i, "firstName", e.target.value)}
                          placeholder={t("step2.firstNamePlaceholder")}
                          style={{ color: "#171717", WebkitTextFillColor: "#171717", backgroundColor: "#ffffff" }}
                      ...
                      <div>
                        <label className="text-xs text-gray-500 mb-1 block">{t("step2.lastName")}</label>
                        <input type="text" value={tr.lastName} onChange={(e) => updateTraveller(i, "lastName", e.target.value)}
                      ...
                      <div>
                        <label className="text-xs text-gray-500 mb-1 block">{t("step2.birthDate")}</label>
                        <input type="date" value={tr.birthDate} onChange={(e) => updateTraveller(i, "birthDate", e.target.value)}
                          style={{ color: "#171717", WebkitTextFillColor: "#171717", backgroundColor: "#ffffff" }}
                      ...
                      <div>
                        <label className="text-xs text-gray-500 mb-1 block">{t("step2.passport")}</label>
                        <input type="text" value={tr.passportNumber} onChange={(e) => updateTraveller(i, "passportNumber", e.target.value)}
                          placeholder={t("step2.passportPlaceholder")}
                          style={{ color: "#171717", WebkitTextFillColor: "#171717", backgroundColor: "#ffffff" }}
                ...
                <div className="flex gap-3">
                  <button onClick={() => setStep(0)}
                    className="flex-1 py-3.5 rounded-xl font-semibold text-base border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors">{t("step2.back")}</button>
                  <button onClick={() => { if (validateTravellers()) setStep(2); }}
                    ...
                    style={{ backgroundColor: "#0EA5E9" }}>{t("step2.next")}</button>
                </div>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <h2 className="text-xl font-bold mb-1" style={{ color: "#0A2342" }}>{t("step3.heading")}</h2>
                <p className="text-sm text-gray-400 mb-6">{t("step3.subtitle")}</p>
                <div className="mb-4">
                  <label className="text-xs text-gray-500 mb-1 block">{t("step3.email")}</label>
                ...
                <div className="border border-gray-100 rounded-xl p-4 mb-4 bg-gray-50">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-4">{t("step3.cardData")}</p>
                  <div className="flex flex-col gap-4">
                    <div>
                      <label className="text-xs text-gray-500 mb-1 block">{t("step3.cardName")}</label>
                      <input type="text" value={payment.cardName} onChange={(e) => setPayment((p) => ({ ...p, cardName: e.target.value }))}
                        placeholder={t("step3.cardNamePlaceholder")}
                        style={{ color: "#171717", WebkitTextFillColor: "#171717", backgroundColor: "#ffffff" }}
                    ...
                    <div>
                      <label className="text-xs text-gray-500 mb-1 block">{t("step3.cardNumber")}</label>
                      <input type="text" value={payment.cardNumber}
                        onChange={(e) => setPayment((p) => ({ ...p, cardNumber: formatCardNumber(e.target.value) }))}
                        placeholder={t("step3.cardNumberPlaceholder")} maxLength={19}
                        style={{ color: "#171717", WebkitTextFillColor: "#171717", backgroundColor: "#ffffff" }}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs text-gray-500 mb-1 block">{t("step3.expiry")}</label>
                        <input type="text" value={payment.expiry}
                          ...
                          placeholder={t("step3.expiryPlaceholder")} maxLength={5}
                          style={{ color: "#171717", WebkitTextFillColor: "#171717", backgroundColor: "#ffffff" }}
                      ...
                      <div>
                        <label className="text-xs text-gray-500 mb-1 block">{t("step3.cvv")}</label>
                        <input type="text" value={payment.cvv}
                          onChange={(e) => setPayment((p) => ({ ...p, cvv: e.target.value.replace(/\D/g, "").slice(0, 4) }))}
                          placeholder={t("step3.cvvPlaceholder")} maxLength={4}
                          style={{ color: "#171717", WebkitTextFillColor: "#171717", backgroundColor: "#ffffff" }}
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={handlePayment} disabled={loading}
                    ...
                    style={{ backgroundColor: "#0EA5E9" }}>
                    {loading ? (
                      <><span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />{t("step3.processing")}</>
                    ) : (
                      <>{t("step3.submit", { total: grandTotal })}</>
                    )}
                  </button>
                </div>
              </div>
          ...
          <div className="w-72 shrink-0">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sticky top-6">
              <h3 className="text-sm font-bold uppercase tracking-wider mb-4" style={{ color: "#0A2342" }}>{t("summary.heading")}</h3>
              ...
              <div className="flex flex-col gap-2 text-sm mb-4 pb-4 border-b border-gray-100">
                ...
                <div className="flex justify-between text-gray-600"><span>× {t(persons === 1 ? "step1.personOne" : "step1.personOther", { count: persons })}</span><span>€{totalPrice}</span></div>
                <div className="flex justify-between text-gray-400 text-xs"><span>{t("summary.serviceFee")}</span><span>€{serviceFee}</span></div>
              </div>
              <div className="flex justify-between font-bold text-base" style={{ color: "#0A2342" }}>
                <span>{t("summary.total")}</span><span>€{grandTotal}</span>
              </div>
              <div className="mt-4 pt-4 border-t border-gray-100 flex flex-col gap-1.5">
                <div className="flex items-center gap-2 text-xs text-gray-400"><span>🔒</span><span>{t("summary.security")}</span></div>
                <div className="flex items-center gap-2 text-xs text-gray-400"><span>✓</span><span>{t("summary.cancellation")}</span></div>
                <div className="flex items-center gap-2 text-xs text-gray-400"><span>✓</span><span>{t("summary.confirmation")}</span></div>
              </div>
            </div>
          </div>
        </div>
