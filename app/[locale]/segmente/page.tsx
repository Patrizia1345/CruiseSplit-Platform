    ...
    <div className="flex flex-col min-h-full font-sans bg-gray-50">
      ...
      <div style={{ backgroundColor: "#0A2342" }} className="px-8 py-8">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-blue-200 text-sm mb-6">{t("subtitlePrefix")}<a href="/routen" className="text-white underline">{t("routesLink")}</a></p>
          <div className="bg-white rounded-2xl p-4 flex flex-wrap gap-3 items-end shadow-lg">
            <div className="flex flex-col gap-1 min-w-[140px] flex-1">
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">{t("search.from")}</label>
              <select value={from} onChange={(e) => setFrom(e.target.value)} className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#0EA5E9] bg-gray-50">
                <option value="Alle">{t("search.allPorts")}</option>
            ...
            <div className="flex flex-col gap-1 min-w-[140px] flex-1">
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">{t("search.to")}</label>
            ...
            <div className="flex flex-col gap-1 min-w-[160px]">
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">{t("search.date")}</label>
            ...
            <div className="flex flex-col gap-1 min-w-[120px]">
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">{t("search.persons")}</label>
              <select value={persons} onChange={(e) => setPersons(Number(e.target.value))} className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#0EA5E9] bg-gray-50">
                {[1, 2, 3, 4].map((n) => <option key={n} value={n}>{t(n === 1 ? "search.personSingular" : "search.personPlural", { count: n })}</option>)}
              </select>
            </div>
      ...
      <div className="max-w-6xl mx-auto w-full px-4 sm:px-8 py-6 flex gap-6 flex-1">
        <aside className="hidden lg:flex w-56 shrink-0 flex-col gap-6">
          <div className="bg-white rounded-2xl shadow-sm p-5">
            <h3 className="text-sm font-bold mb-4 uppercase tracking-wider" style={{ color: "#0A2342" }}>{t("filter.priceHeading")}</h3>
            <div className="flex flex-col gap-2">
              ...
              <p className="text-xs text-gray-500">{t("filter.priceLabel", { max: priceMax })}</p>
            </div>
          </div>
          <div className="bg-white rounded-2xl shadow-sm p-5">
            <h3 className="text-sm font-bold mb-4 uppercase tracking-wider" style={{ color: "#0A2342" }}>{t("filter.durationHeading")}</h3>
            <div className="flex flex-col gap-2">
              {DURATION_OPTIONS.map((opt) => {
                const labelKey =
                  opt === "Alle" ? "filter.durationAll" :
                  opt === "2 Tage" ? "filter.duration2" :
                  opt === "3 Tage" ? "filter.duration3" :
                  "filter.duration5";
                return (
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5">
            <h3 className="text-sm font-bold mb-4 uppercase tracking-wider" style={{ color: "#0A2342" }}>{t("filter.cabinHeading")}</h3>
            <div className="flex flex-col gap-2">
              {CABIN_TYPES.map((c) => <FilterCheckbox key={c} label={c} checked={selectedCabinFilters.has(c)} onChange={() => toggleCabinFilter(c)} />)}
            </div>
          </div>
          <div className="bg-white rounded-2xl shadow-sm p-5">
            <h3 className="text-sm font-bold mb-4 uppercase tracking-wider" style={{ color: "#0A2342" }}>{t("filter.shippingHeading")}</h3>
            <div className="flex flex-col gap-2">
        ...
        <div className="flex-1 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-500">{t(results.length === 1 ? "result.countOne" : "result.countOther", { count: results.length })}</p>
            <select className="text-sm border border-gray-200 rounded-xl px-3 py-2 text-gray-600 focus:outline-none focus:ring-2 focus:ring-[#0EA5E9] bg-white">
              <option>{t("sort.cheapest")}</option>
              <option>{t("sort.fastest")}</option>
              <option>{t("sort.recommended")}</option>
            </select>
          </div>
          ...
          {results.length === 0 && (
            <div className="flex flex-col items-center justify-center flex-1 py-24 text-center">
              ...
              <p className="text-gray-500 text-lg font-medium">{t("empty.title")}</p>
