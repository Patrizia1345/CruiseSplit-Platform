...
function RhineMap({ activeSegment }: { activeSegment: number | null }) {
  const t = useTranslations("viking");
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="font-bold text-base" style={{ color: "#0A2342" }}>{t("map.heading")}</h3>
          <p className="text-xs text-gray-400 mt-0.5">{t("map.subtitle")}</p>
        </div>
        {active && (
    ...
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col justify-center px-12">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <span className="text-white font-bold text-lg">{t("airline")}</span>
              <span className="text-white/60 text-xs block">{t("demo")}</span>
            </div>
          </div>
          <h1 className="text-4xl font-bold text-white mb-2">{t("heading")}</h1>
          <p className="text-white/80 text-lg mb-4">{t("subtitle")}</p>
          <div className="flex gap-3 flex-wrap">
            {[
              { key: "duration", label: t("tags.duration") },
              { key: "countries", label: t("tags.countries") },
              { key: "segments", label: t("tags.segments") },
              { key: "excursion", label: t("tags.excursion") },
            ].map((tag) => (
              <span key={tag.key} className="text-xs px-3 py-1.5 rounded-full text-white border border-white/30 bg-white/10">
                {tag.label}
              </span>
            ))}
          </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 py-6 flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-72 shrink-0 flex flex-col gap-4">
          ...
          {/* Personen */}
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <h3 className="text-sm font-bold mb-3" style={{ color: "#0A2342" }}>{t("sidebar.personsHeading")}</h3>
          ...
          <div className="rounded-2xl border p-4 text-xs" style={{ backgroundColor: "#FFF5F5", borderColor: "#FCA5A5" }}>
            <p className="font-bold mb-1" style={{ color: "#B22222" }}>{t("sidebar.infoHeading")}</p>
            <p className="text-gray-600">{t("sidebar.infoText")}</p>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            ...
            <p className="text-sm text-gray-400">{t("segmentsSubtitle")}</p>
          </div>
              ...
              <div className="p-4 flex flex-col gap-3">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex flex-col items-end gap-2 shrink-0">
                    ...
                    <span className="text-xs text-gray-400">{t("segmentCard.perPerson")}</span>
      ...
      <div className="border-t border-gray-200 bg-white mt-6 px-8 py-8">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div>
            <h3 className="font-bold text-lg" style={{ color: "#0A2342" }}>{t("footer.heading")}</h3>
            <p className="text-gray-500 text-sm">{t("footer.text")}</p>
          </div>
