...
type NavItem = { id: string; labelKey: string; icon: string };
...
const NAV_ITEMS: NavItem[] = [
  { id: "uebersicht", labelKey: "nav.overview", icon: "◼" },
  { id: "buchungen", labelKey: "nav.bookings", icon: "📋" },
  { id: "segmente", labelKey: "nav.segments", icon: "🗺" },
  { id: "inventar", labelKey: "nav.inventory", icon: "📦" },
  { id: "einstellungen", labelKey: "nav.settings", icon: "⚙" },
];
...
const KPI_CARDS = [
  { labelKey: "kpi.bookingsToday", value: "12", unit: "", trendKey: "kpi.bookingsTodayTrend", up: true, icon: "📋" },
  { labelKey: "kpi.occupancy", value: "78", unit: "%", trendKey: "kpi.occupancyTrend", up: true, icon: "📊" },
  { labelKey: "kpi.revenueToday", value: "4.320", unit: "€", trendKey: "kpi.revenueTodayTrend", up: true, icon: "💶" },
  { labelKey: "kpi.openCabins", value: "23", unit: "", trendKey: "kpi.openCabinsTrend", up: false, icon: "🛏" },
];
...
function StatusBadge({ status }: { status: BookingStatus }) {
  ...
  const labels = {
    "Bestätigt": t("status.confirmed"),
    "Ausstehend": t("status.pending"),
  ...
  return (
    <>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {KPI_CARDS.map((card) => (
          <div key={card.labelKey} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex flex-col gap-3">
            <div className="flex items-start justify-between">
              <span className="text-sm text-gray-500 font-medium">{t(card.labelKey)}</span>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="font-bold text-base" style={{ color: "#0A2342" }}>{t("bookings.recentHeading")}</h2>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 text-gray-400 text-xs uppercase tracking-wider">
                  <th className="px-6 py-3 text-left font-semibold">{t("bookings.tableId")}</th>
                  <th className="px-6 py-3 text-left font-semibold">{t("bookings.tableSegment")}</th>
                  <th className="px-6 py-3 text-left font-semibold">{t("bookings.tablePassenger")}</th>
                  <th className="px-6 py-3 text-right font-semibold">{t("bookings.tablePrice")}</th>
                  <th className="px-6 py-3 text-left font-semibold">{t("bookings.tableStatus")}</th>
                </tr>
              </thead>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col">
          <h2 className="font-bold text-base mb-1" style={{ color: "#0A2342" }}>{t("bookings.weeklyHeading")}</h2>
          <p className="text-xs text-gray-400 mb-6">{t("bookings.weeklySubtitle")}</p>
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 flex items-center gap-4 text-xs text-gray-400">
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm" style={{ backgroundColor: "#0EA5E9" }} />{t("bookings.today")}</span>
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm" style={{ backgroundColor: "#0A2342", opacity: 0.2 }} />{t("bookings.previousDays")}</span>
          </div>
        </div>
      </div>
    </>
...
function BuchungenView() {
  ...
  const filterLabels: Record<"Alle" | BookingStatus, string> = {
    "Alle": t("bookings.filterAll"),
    "Bestätigt": t("status.confirmed"),
    "Ausstehend": t("status.pending"),
    "Storniert": t("status.cancelled"),
  };
    ...
    <div className="flex gap-6 flex-1">
      <div className="flex-1 flex flex-col gap-4">
        ...
        <div className="flex gap-3 flex-wrap">
          {[
            { label: t("bookings.summaryTotal"), value: filtered.length, color: "#0A2342" },
            { label: t("bookings.summaryConfirmed"), value: confirmed, color: "#059669" },
            { label: t("bookings.summaryPending"), value: pending, color: "#D97706" },
            { label: t("bookings.summaryCancelled"), value: cancelled, color: "#DC2626" },
            { label: t("bookings.summaryRevenue"), value: `€${totalRevenue.toLocaleString("de-DE")}`, color: "#0EA5E9" },
          ].map((pill) => (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex-1">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 text-gray-400 text-xs uppercase tracking-wider">
                  <th className="px-6 py-3 text-left font-semibold">{t("bookings.detailsBookingNumber")}</th>
                  <th className="px-6 py-3 text-left font-semibold">{t("bookings.detailsSegment")}</th>
                  <th className="px-6 py-3 text-left font-semibold">{t("bookings.detailsCabin")}</th>
                  <th className="px-6 py-3 text-left font-semibold">{t("bookings.detailsPassenger")}</th>
                  <th className="px-6 py-3 text-left font-semibold">{t("bookings.detailsDate")}</th>
      ...
      {selectedBooking && (
        <div className="w-72 shrink-0 bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex flex-col gap-4 self-start sticky top-0">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-sm" style={{ color: "#0A2342" }}>{t("bookings.detailsHeading")}</h3>
            <button onClick={() => setSelectedBooking(null)} className="text-gray-400 hover:text-gray-600 text-lg leading-none">×</button>
          </div>

          <div className="flex flex-col gap-1">
            <span className="text-xs text-gray-400">{t("bookings.detailsBookingNumber")}</span>
          ...
          {[
            { label: t("bookings.detailsSegment"), value: selectedBooking.segment },
            { label: t("bookings.detailsPassenger"), value: selectedBooking.passenger },
            { label: t("bookings.detailsCabin"), value: selectedBooking.cabin },
            { label: t("bookings.detailsDate"), value: selectedBooking.date },
            { label: t("bookings.detailsPersons"), value: `${selectedBooking.persons}` },
            { label: t("bookings.detailsPricePerPerson"), value: `€${selectedBooking.price}` },
            { label: t("bookings.detailsTotal"), value: `€${selectedBooking.price * selectedBooking.persons}` },
          ].map((row) => (
          ...
          <div>
            <span className="text-xs text-gray-400 block mb-2">{t("bookings.changeStatus")}</span>
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside className="w-60 shrink-0 flex flex-col h-full shadow-lg" style={{ backgroundColor: "#0A2342" }}>
          <div className="px-6 py-6 border-b border-white/10">
            <span className="text-white font-bold text-lg leading-tight block">{t("brand")}</span>
            <span className="text-xs font-medium" style={{ color: "#0EA5E9" }}>{t("subtitle")}</span>
          </div>
          ...
          <header className="bg-white border-b border-gray-100 px-8 py-4 flex items-center justify-between shrink-0">
            <div>
              <h1 className="text-xl font-bold" style={{ color: "#0A2342" }}>
                {activeNav === "uebersicht" && t("nav.overview")}
                {activeNav === "buchungen" && t("nav.bookings")}
                {activeNav === "segmente" && t("nav.segments")}
                {activeNav === "inventar" && t("nav.inventory")}
                {activeNav === "einstellungen" && t("nav.settings")}
              </h1>
          ...
          <main className="flex-1 overflow-y-auto px-8 py-6 flex flex-col gap-6">
            ...
            {(activeNav === "segmente" || activeNav === "inventar" || activeNav === "einstellungen") && (
              <div className="flex flex-col items-center justify-center flex-1 py-24 text-center">
                ...
                <p className="text-gray-500 text-lg font-medium">{t("comingSoon.title")}</p>
                <p className="text-gray-400 text-sm mt-1">{t("comingSoon.description")}</p>
              </div>
            )}
          </main>
        </div>
